...
import dataone from './Navbar.json';

const Navbarone = ({ data }) => {
    ...
    <div className='navbar'>
      <Navbar expand="md" className="navbar-custom">
        <Container>
          ...
          <Navbar.Toggle className="d-md-none" onClick={() => setIsOpen(!isOpen)}>
            <FaBars />
          </Navbar.Toggle>
          
          {/* Hide this Nav on mobile */}
          <Navbar.Collapse id="navbar-nav" className={`d-none d-md-flex ${isOpen ? 'show' : ''}`}>
            <Nav className="ml-auto">
              {dataone.map((item) => (
                <Nav.Link href={item.link} className="nav-link-custom" key={item.id}>{item.name}</Nav.Link>
              ))}
              <Nav.Link className="nav-link-custom" >Sign in</Nav.Link>
              <Nav.Link className="nav-link-custom">Sign up</Nav.Link>
